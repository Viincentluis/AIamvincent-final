...
export default function AIamVincent() {
  ...
  const [messages, setMessages] = useState([
    { role: 'bot', text: 'Hallo! Ich bin Viincentluis. Stellen Sie mir gerne Fragen – ich antworte authentisch basierend auf meinen Erfahrungen bei PwC und meinen Projekten im Energiesektor. Was möchten Sie gerne wissen?' }
  ]);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const systemPrompt = `Du bist der digitale Zwilling von Viincentluis. Antworte in der Ich-Perspektive, warm, aufgeschlossen und optimistisch. Stütze dich AUSSCHLIESSLICH auf die bereitgestellte Wissensbasis.
...
- Unter Druck: fokussiert, priorisiere`;
      ...
      setMessages(prev => [...prev, { 
        role: 'bot', 
        text: `Entschuldigung, es gab einen Fehler: ${error.message}` 
      }]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Stellen Sie Fragen an Viincentluis" />
      ...
      <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif', height: '100vh', display: 'flex', flexDirection: 'column', background: '#f5f5f5' }}>
        ...
        {screen === 'password' && (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
            <div style={{ background: 'white', padding: '40px', borderRadius: '16px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', width: '100%', maxWidth: '400px', margin: '20px' }}>
              ...
              <p style={{ textAlign: 'center', color: '#666', marginBottom: '30px', fontSize: '14px' }}>Stellen Sie Fragen an Viincentluis</p>
        ...
        {screen === 'apikey' && (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
            <div style={{ background: 'white', padding: '40px', borderRadius: '16px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', width: '100%', maxWidth: '450px', margin: '20px' }}>
              ...
              <p style={{ color: '#666', marginBottom: '20px', fontSize: '13px' }}>Geben Sie Ihren Claude API Key ein.</p>
        ...
        {screen === 'chat' && (
          <>
            <div style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white', padding: '24px', textAlign: 'center' }}>
              <div style={{ fontSize: '24px', fontWeight: '600', marginBottom: '8px' }}>AIamvincent.de</div>
              <div style={{ fontSize: '13px', opacity: 0.9 }}>Digitaler Zwilling – Viincentluis</div>
            </div>
